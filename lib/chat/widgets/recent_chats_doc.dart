...
import '../chatscreenDoc.dart';
import '../models/message_model.dart';


class RecentChatsDoc extends StatefulWidget{

  final String Cid;
  const RecentChatsDoc({super.key, required this.Cid});

  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _RecentChatsDocState(Cid: Cid);
  }

}

class _RecentChatsDocState extends State<RecentChatsDoc>{

  final String Cid;
  _RecentChatsDocState({ required this.Cid});
        ...
        child: Container(
          ...
          child: ClipRRect(
            ...
            child: GestureDetector(
                onTap: () async {
                  if(pending['sender'] != Cid){
                      final snapshot = await FirebaseDatabase.instance.ref().child('patients').child(pending['sender']).get();
                      ...
                      Navigator.push(context, MaterialPageRoute(builder: (_){
                        return ChatScreenDoc(user: user, Cid: Cid,);
                      }));

                  }else{
                    final snapshot = await FirebaseDatabase.instance.ref().child('patients').child(pending['receiver']).get();
                    Map user = snapshot.value as Map;
  ...
  Widget build(BuildContext context) {
    ...
    Query dbRef1 = FirebaseDatabase.instance.ref().child('doctors').child(Cid).child('recent');
