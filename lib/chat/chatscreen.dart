...
class ChatScreen extends StatefulWidget {
  ChatScreen({required this.user, required this.Cid,});
  Map user;
  String Cid;
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return _ChatScreenState(user: user, Cid: Cid,);
  }
}
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    Query dbRef1 = FirebaseDatabase.instance.ref().child('patients').child(Cid).child('messages').child(user['Id']);

    return Scaffold(
      backgroundColor: Colors.white,
      appBar: AppBar(
        title: Text(
          'Dr. '+user['first name']+" "+user['second name'],
              ...
              onPressed: (){
                if(_messageTextEditingController.text.isNotEmpty){
                  ...
                  if(databaseRef != null){
                    ...
                    FirebaseDatabase.instance.ref().child('doctors').child(user['Id']).child("messages").child(Cid)
                    ...
                    FirebaseDatabase.instance.ref().child('patients').child(Cid).child("messages").child(user['Id'])
                    ...
                    FirebaseDatabase.instance.ref().child('doctors').child(user['Id']).child('peers').child(Cid).set({
                      'peer': Cid,
                    });

                    FirebaseDatabase.instance.ref().child('patients').child(Cid).child('peers').child(user['Id']).set({
                      'peer': user['Id'],
                    });
